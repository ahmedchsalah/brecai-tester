...
export const ENDPOINT_GROUPS: EndpointGroup[] = [
...

  {
    ...
    accent: 'cyan',
    endpoints: [
      { id: 'admin-ai-list', method: 'GET', path: '/admin/ai-models', label: 'List AI Models', description: 'List all registered AI models with performance metrics.' },
      { id: 'admin-ai-show', method: 'GET', path: '/admin/ai-models/:id', label: 'Show AI Model', description: 'Model details with performance metrics and FL round history.', pathParams: [{ key: 'id', example: '1' }] },
      {
        id: 'admin-ai-create', method: 'POST', path: '/admin/ai-models', label: 'Create AI Model', description: 'Register a new model. Models are stored on HuggingFace — this is metadata only.',
        defaultBody: {
          name: 'BReCAI-A6 Cross-Attention Fusion',
          slug: 'a6_fusion',
          version: 'v12.5',
          inference_type: 'a6_fusion',
          description: 'Full fusion model (CONCH + 19 clinical features). Best accuracy.',
          auc: 0.9016,
          accuracy: 0.852,
          sensitivity: 0.890,
          specificity: 0.814,
      ...
      {
        id: 'admin-ai-update', method: 'PUT', path: '/admin/ai-models/:id', label: 'Update AI Model', description: 'Update model metadata or performance metrics.',
        pathParams: [{ key: 'id', example: '1' }],
        defaultBody: { auc: 0.930, accuracy: 0.885, is_active: true },
      },
      ...
      { id: 'admin-ai-activate', method: 'POST', path: '/admin/ai-models/:id/activate', label: 'Activate Model', description: 'Activate model, deactivate all others of the same type.', pathParams: [{ key: 'id', example: '1' }] },
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        label: '🏗 Extract CONCH Features',
        description: 'Simulate calling FastAPI /extract. In a real environment, this extracts patches from the WSI and sends them to the AI service to generate a .pt feature file. Set simulate=1 to bypass and create a mock feature file.',
        pathParams: [{ key: 'id', example: '1' }],
        queryParams: { simulate: '1' }
      },

      { id: 'doc-predictions', method: 'GET', path: '/doctor/predictions', label: 'List Predictions', description: 'All AI predictions for this doctor.', queryParams: { status: '', examination_id: '', patient_id: '' } },
      { id: 'doc-predictions-show', method: 'GET', path: '/doctor/predictions/:id', label: 'Show Prediction', description: 'Single prediction with XAI results.', pathParams: [{ key: 'id', example: '1' }] },
      { id: 'doc-predictions-status', method: 'GET', path: '/doctor/predictions/:id/status', label: 'Prediction Status', description: 'Poll prediction job status.', pathParams: [{ key: 'id', example: '1' }] },
      {
        ...
        description: 'Dispatch AI prediction job. Packages clinical data + WSI (if any) and sends to FastAPI. Status starts as "pending" then moves to "processing" → "completed".',
        defaultBody: { examination_id: 1, wsi_upload_id: null, ai_model_id: 1 },
      },
      { id: 'doc-predict-retry', method: 'POST', path: '/doctor/predictions/:id/retry', label: 'Retry Failed Prediction', description: 'Re-dispatch a failed prediction job.', pathParams: [{ key: 'id', example: '1' }] },
      { id: 'doc-xai', method: 'GET', path: '/doctor/xai-results/:prediction_id', label: 'XAI Result', description: 'Explainability heatmap + SHAP values for a prediction.', pathParams: [{ key: 'prediction_id', example: '1' }] },
...

  {
    ...
    endpoints: [
      ...
      { id: 'fl-models-show', method: 'GET', path: '/fl/models/:id', label: 'Show FL Model', description: 'Single FL model.', pathParams: [{ key: 'id', example: '1' }] },
      {
        ...
        defaultBody: { name: 'BrCa-FL-v1', version: '1.0.0', slug: 'fl_v1', inference_type: 'a6_fusion', auc: 0.900, n_checkpoints: 5, is_active: false },
      },
      {
        id: 'fl-models-update', method: 'PUT', path: '/fl/models/:id', label: 'Update FL Model', description: 'Update FL model metadata.',
        pathParams: [{ key: 'id', example: '1' }],
        defaultBody: { name: 'BrCa-FL-v1-Updated', version: '1.0.1', auc: 0.910 },
      },
  ...
  // ── BReCAI FastAPI Microservice — Direct Test Group ───────────────────────
  {
    id: 'brecai-fastapi',
    label: 'BReCAI FastAPI (HF Space)',
    icon: '🔬',
    ...
    endpoints: [
      {
        ...
        path: '/health',
        label: '✅ Health Check',
        description: 'Check if the FastAPI space is alive and all checkpoints are loaded. Should return models_loaded: true, a6_checkpoints: 15.',
      },
      {
        id: 'ai-models',
        method: 'GET',
        ...
        description: 'List all A6 and A4 checkpoint filenames currently loaded in the inference engine.',
      },
      {
        id: 'ai-predict-clinical',
        method: 'POST',
        path: '/predict/clinical',
        label: '🩺 Clinical-Only Prediction (DZ Mode)',
        description: 'Fast screening without WSI. Uses only the clinical encoder branch of A6 across all 15 checkpoints. Ideal for Algeria clinics with no genomics data.',
        defaultBody: {
          clinical: {
            er_status: 1,
            pr_status: 1,
      ...
      {
        id: 'ai-predict-clinical-full',
        method: 'POST',
        path: '/predict/clinical',
        label: '🧬 Clinical-Only Prediction (FULL Mode)',
        description: 'Clinical screening using real genomics data (Hypoxia scores + FGA). Higher accuracy than DZ mode.',
        defaultBody: {
          clinical: {
            er_status: 1,
            pr_status: 1,
            ...
            pr_status_missing: 0,
            her2_binary_missing: 0,
            buffa_hypoxia_score: -8.5,
            ragnum_hypoxia_score: -5.2,
            winter_hypoxia_score: -6.1,
            fraction_genome_altered: 0.12,
            tumor_break_load: 3.4,
            pr_pct_score: 0.8,
            is_ductal: 1,
            is_lobular: 0,
          },
